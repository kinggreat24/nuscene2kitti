...
MetaDataReader::loadFromDirectory(const fs::path& directoryPath)
{
  ...
  const fs::path calibratedSensorFile = directoryPath / "calibrated_sensor.json";
  ...
  calibratedSensorToken2CalibratedSensorInfo = loadCalibratedSensorInfo(calibratedSensorFile);
  ...
  for (const auto& keyvalue : scene2Samples) {
    ...
    for (const auto& sampleInfo : sampleInfos) {
      for (const auto& sampleData : sample2SampleData[sampleInfo.token]) {
        ...
        // add egoPoseInfo(读取所有数据的姿态)
        egoPoseToken2sceneToken.emplace(sampleData.egoPoseToken, sceneToken);

        // add calibrated sensor info(读取所有数据的标定参数，包括相机内参，传感器到车体的坐标变换等)
...
egoPoseJson2EgoPoseInfo(const json::json& egoPoseJson)
{
  ...
  egoPoseInfo.token     = egoPoseJson["token"];
  return egoPoseInfo;
}
...
MetaDataReader::loadCalibratedSensorInfo(const fs::path& filePath)
{
  ...
  for (const auto& calibratedSensorJson : calibratedSensorJsons) {
    ...
    boost::optional<json::json> sensor_intrinsics = calibratedSensorJson["camera_intrinsic"];

    boost::optional<IntrinsicsMatrix> cam_intrinsics_op = boost::none;
    if(sensor_intrinsics.get().size())
    {
      IntrinsicsMatrix cam_intrinsics;
      cam_intrinsics.at(0).at(0) = sensor_intrinsics.get()[0][0];
      cam_intrinsics.at(1).at(1) = sensor_intrinsics.get()[1][1];
      cam_intrinsics.at(0).at(2) = sensor_intrinsics.get()[0][2];
      cam_intrinsics.at(1).at(2) = sensor_intrinsics.get()[1][2];

      cam_intrinsics_op = boost::optional<IntrinsicsMatrix>(cam_intrinsics);
    }
    ...
    calibratedSensorToken2CalibratedSensorInfo.emplace(token, calibratedSensorInfo);
