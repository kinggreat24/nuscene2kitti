...
std::vector<float>
readBinaryPcdFile(const fs::path& filePath)
{
  std::vector<float> fileValues;
  try {
    std::ifstream fin(filePath.string(), std::ios::binary);
    fileValues = readBinaryPcdFile(fin);
...
void 
writeBinaryPcdFile(const fs::path& filePath, std::vector<float>& binFileValues)
{
  std::ofstream outBinFile(filePath.string(), ios::out | ios::binary);
